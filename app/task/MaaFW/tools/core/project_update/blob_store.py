...
Windows 11 本机实测（2026-09-23：库里的 blob 与视图名 A、B 是同一 inode 的三个硬链接，
两个进程分别经 A、B 加载这个 DLL 并常驻）：A、B 两个名字都删得掉——包括进程当初经由
加载的那个——删到只剩最后一个链接（库里的 blob）时才报拒绝访问（WinError 5）。被锁的
是 inode 的**最后一个链接**，与进程经由哪个名字加载无关。所以「A 在跑、B 换掉自己那份
同内容的链接」不会被挡住，只要库里的 blob 还在；而 blob 在还有视图 / 载荷引用时本来就
不删（回收只删 ``st_nlink == 1`` 的）。``os.replace`` 覆盖与以写方式打开没有单独实验，
不要据此推断。仍保留兜底：先 rename 再删、删不掉留给启动期清理（旧系统版本、杀软持
句柄这类情况实验覆盖不到）。回收在启动期做：``st_nlink == 1`` 的 blob 没有任何副本 /
载荷引用，删掉。
"""
