...
def _copy_exclusive(source: Path, destination: Path) -> None:
    """复制成一个**新**文件：目标已存在就报错，绝不以 ``wb`` 打开一个已有的（可能是
    载荷硬链接的）文件。"""
...
def place_fresh(source: Path, destination: Path, *, link: bool) -> str:
    """把 ``source`` 放到 ``destination``：先删掉目标，再链接（``link=True``）或复制。

    返回 ``"linked"`` / ``"copied"``。``copy`` 只在链接失败时兜底，且以独占方式新建。
    """
...
def build_from_source(
    ...
    *,
    ...
    产物文件的 ``origin`` 全部是 ``import``（登记时的默认值）。
    """
