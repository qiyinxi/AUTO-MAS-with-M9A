...
export default {
  ...
  edit: {
    ...
    maaCultivatePreviewStageHeading: '刷取计划',
    maaCultivatePreviewDemandHeading: '材料需求',
    maaCultivatePreviewUnobtainable: '以下材料无法通过刷取获得，需在游戏内另行获取',
    maaCultivatePreviewNone: '暂无待刷取条目（材料已齐或当前无开放关卡）',
    maaCultivateSanityUnit: '理智',
    maaCultivatePreviewSanityTotal: '预计理智合计（不含固定产出关）',
    maaCultivateMissingProgression: '干员识别数据',
    maaCultivateMissingInventory: '仓库识别数据',
    ...
    maaCustomInfrastPlanHintPeriod: '排班表含时间段，由 MAA 按时间段自动换班，不支持手选班次',
    ...
    maaEndResolutionOriginal: '恢复原状（{displayType} {resolution}）',
    maaEndResolutionRestoreOriginal: '恢复原状',
    ...
    resetManagedOverridesHint: '清除你在 MAS 里改过的所有覆盖值，重新读取 SRA / 三月七当前配置',
    resetManagedOverridesConfirmTitle: '重置为源配置？',
    resetManagedOverridesConfirmDesc:
      '会删除这个用户在 MAS 里改过的全部覆盖值（所有模块、所有字段），之后各项都按 SRA / 三月七当前配置显示与运行。源配置文件本身不会被修改，此操作无法撤销。',
    ...
    hsrSharedEngineSwitchHint:
      '这里改的是脚本级引擎分配：本脚本所有「脚本」来源用户，以及「用户」来源里没有单独指定引擎的用户，都会改用这个引擎。切换后该引擎自己的一套原生配置项和体力副本生效。',
    ...
    hsrUseScriptShared: '本脚本下所有「脚本」来源用户共用一份任务配置。',
    hsrSharedPlanHint: '当前编辑的是脚本共享任务配置，会影响本脚本下所有「脚本」来源用户。',
    ...
    directEngineSra: 'SRA',
    directEngineM7a: '三月七',
    directEngineDescSra: '运行 SRA 当前选中的配置文件。',
    directEngineDescM7a: '运行三月七当前的 config.yaml。',
    ...
    important: '重要提示',
    basicInfo: '基本信息',
    maaEndConfigActions: '配置操作',
    maaEndConfiguring: '正在配置',
    maaEndConfigureSource: '配置{source}',
    maaEndGroupEnabled: '已启用 {n} / {m} 项任务',
    maaEndEnableGroup: '整组启用',
    ...
    managedOverridesReset: '已清除全部覆盖值，现在按 SRA / 三月七当前配置显示与运行',
    ...
    noSraMarch7thAssistant: '当前没有可用的 SRA / 三月七',
    currentVersion: '当前版本',
    ...
    hsrUpdateSectionHint:
      '三月七与 SRA 是您自行安装的第三方工具，MAS 默认不会改写它们的目录。自动更新只在本轮任务全部跑完后进行，不会让当前任务等待下载；首次开启后要跑满一轮才会生效，想马上更新请用下方的手动按钮。',
    hsrUpdateModeTip:
    ...
    hsrUpdateM7ASource: '三月七下载源',
    hsrUpdateM7ASourceTip:
      '三月七只提供 GitHub 与 Mirror 酱两个下载源。选择 Mirror 酱时必须填写 CDK，CDK 不可用会报错跳过，不会自动改走 GitHub',
    ...
    hsrUpdateCdkTip:
      '只用于三月七 / SRA 的更新，与全局设置中的 CDK 无关；任一下载源选择 Mirror 酱时必须填写',
    ...
    hsrUpdateNeedPath: '填写三月七或 SRA 路径后，这里会出现对应的检查 / 更新按钮',
    hsrUpdateEngineM7A: '三月七',
    ...
    maafwCheckboxCountExact: '需选择 {count} 项',
    maafwCheckboxCountMin: '至少选择 {min} 项',
    maafwCheckboxCountMax: '最多选择 {max} 项',
    maafwCheckboxCountCurrent: '（当前 {count} 项）',
    ...
    maafwDirectoryLockedHint:
      '项目已导入到 AUTO-MAS 自己的目录，运行与更新都在这里，来源目录可以删掉；要换项目请新建脚本',
    ...
    bettergiTeamOffTip: '队伍配置已关闭：表格数据保留，但除通用队伍外不参与选队。',
    bettergiTeamAdd: '添加队伍',
    bettergiTeamEditTitle: '编辑队伍',
    bettergiTeamBatchDelete: '批量删除',
    bettergiTeamOpenStrategyDir: '打开策略目录',
    bettergiTeamAdvanced: '高级设置',
    bettergiTeamDragTip: '拖动左侧手柄可调整顺序；第 0 行通用队伍固定在最前',
    bettergiTeamIndexColumn: '序号',
    bettergiTeamNameColumn: '队伍名称',
    bettergiTeamStrategyColumn: '战斗策略',
    bettergiTeamScenesColumn: '战斗场景',
    bettergiTeamNoteColumn: '备注',
    bettergiTeamActionColumn: '操作',
    bettergiTeamGeneralTag: '通用',
    bettergiTeamGeneralScene: '通用兜底',
    bettergiTeamGeneralHint: '兜底所有未命中场景的战斗任务，不可删除',
    bettergiTeamStrategyFollowGeneral: '跟随通用策略',
    bettergiTeamNoScene: '未指定（不参与选队）',
    bettergiTeamEditScenes: '战斗场景',
    bettergiTeamEdit: '编辑',
    bettergiTeamDuplicate: '复制',
    bettergiTeamDelete: '删除',
    bettergiTeamEmpty: '暂无队伍，点击「添加队伍」开始配置',
    bettergiTeamStrategyTip: '留空表示跟随「通用战斗策略」；填写则命中该场景时使用此策略',
    bettergiTeamSceneTip: '可跨标签页同时勾选多个条件；未填写任何条件时该队伍不参与选队',
    bettergiTeamSceneDomainTab: '自动秘境',
    bettergiTeamSceneLeylineTab: '自动地脉花',
    bettergiTeamSceneBossTab: '自动首领讨伐',
    bettergiTeamSceneRegion: '地区',
    bettergiTeamSceneDomain: '秘境',
    bettergiTeamSceneReward: '奖励物品（仅展示）',
    bettergiTeamSceneCountry: '地区',
    bettergiTeamSceneLeylineType: '地脉类型',
    bettergiTeamSceneBoss: '首领',
    bettergiTeamSceneAddDomain: '添加秘境条件',
    bettergiTeamSceneAddLeyline: '添加地脉条件',
    bettergiTeamSceneAddBoss: '添加首领条件',
    bettergiTeamSceneDialogTip: '匹配为精确匹配：只有与本次实际执行目标完全一致的队伍才会被选中。',
    bettergiTeamNameRequired: '请填写队伍名称',
    bettergiTeamNameExists: '已存在同名队伍',
    ...
    zzzodSlotsLoadFailed: '加载实例槽失败',
    zzzodSlotTabSlots: '实例槽',
    zzzodSlotTabRecycle: '回收池',
    zzzodSlotColIdx: '槽号',
    zzzodSlotColKind: '类别',
    zzzodSlotColOwner: '归属',
    zzzodSlotColDir: '目录',
    zzzodSlotColSize: '占用',
    ...
    zzzodRecycleColSlot: '槽号',
    zzzodRecycleColKind: '类别',
    zzzodRecycleColTime: '归档时间',
    zzzodRecycleColFiles: '文件',
    zzzodRecycleColSize: '占用',
    zzzodRecycleColOps: '操作',
    zzzodRecycleKindSlot: '槽内容',
    zzzodRecycleKindMas: '备份池',
    zzzodRecycleHint:
      '被删除的用户/脚本与手动清理掉的槽内容都归档在这里（按槽号归池、跨脚本共享）；「恢复」把快照放回某个用户的绑定槽（也可新建一个用户接收），用户还在时想回滚配置请用该用户页的「配置恢复」。',
    ...
    zzzodRecycleRestoreConfirm:
      '把这份快照（槽 {slot}）恢复到某个用户的槽？目标用户已有绑定槽时，其当前内容会先归档存底。',
    zzzodRecycleRestoreTargetHint: '快照：槽 {slot} · {ts}',
    zzzodRecycleRestoreToUser: '恢复到现有用户',
    zzzodRecycleRestoreToNewUser: '新建一个用户',
    zzzodRecycleRestoreUserPlaceholder: '选择用户',
    zzzodRecycleRestoreNewUserName: '新用户名称',
    zzzodRecycleRestoreNewUserNameDefault: '恢复的用户',
    zzzodRecycleRestoreOverwriteHint:
      '该用户当前绑定槽 {slot}，恢复会替换它的内容（替换前先归档存底，可从回收池找回）。',
    zzzodRecycleRestoreUserRequired: '请先选择目标用户',
    zzzodRecycleRestoreNameRequired: '请填写新用户名称',
    zzzodRecycleRestoreDone: '恢复完成',
  ...
  scripts: {
    ...
    create: {
      ...
      mfwSourceHeadingDesc:
        '已经导入过的项目可以直接复用，同一个项目开几个脚本都不用再选目录；也可以新建一个别的项目。',
      mfwNewProject: '新建一个别的项目：选择本地目录',
      mfwNewProjectDesc: '进入引导页选一个带 interface.json 的项目目录导入',
      mfwReuse: '复用已导入的项目',
      mfwReuseDesc: '直接沿用那个脚本的项目：运行时与模型文件共用、不另占空间；用户与运行设置不带',
      mfwReuseFrom: '来自脚本「{name}」',
      mfwReuseFromMany: '来自脚本「{name}」等 {count} 个',
      mfwReuseLoading: '正在读取已导入的项目...',
      mfwReuseEmpty: '还没有导入过 {type} 项目',
      mfwReuseBusy: '运行中',
  ...
  setting: {
    ...
    notify: {
      ...
      openclawQqBound: '已连接',
      openclawQqConnecting: '连接中',
      openclawQqReconnecting: '正在重连',
      ...
      channelsGroup: '内置',
      customGroup: '自定义',
      countTemplate: '已激活 {n} 个',
      activeEmpty: '还没有启用中的渠道，点下方渠道填好配置再启用',
      idleSection: '未激活',
      idleEmpty: '所有渠道都已启用',
      customSection: '自定义渠道',
      customEmpty: '暂无自定义渠道',
      ...
      manage: '点击管理',
      webhookCount: '{n} 条',
      instantHint: '改动即时生效，无需保存',
      bindStatus: '绑定状态',
      loadFailed: '通知渠道描述加载失败',
      retryLoad: '重试',
      koishiToken: 'Koishi Token',
      koishiWsPlaceholder: 'ws://localhost:5140/AUTO_MAS',
      cmccNewMsgName: '中国移动5G短信',
      webhookName: '自定义 Webhook',
      webhookDesc: '自己拼 URL、请求头与消息模板的推送目标，与内置渠道并行发送',
      summary: {
        system: '使用系统弹窗推送任务通知',
        mail: 'SMTP: {SMTPServerAddress} · 收信: {ToAddress}',
        mailEmpty: '邮箱配置不完整',
        serverchan: 'SendKey 已配置',
        serverchanEmpty: 'SendKey 未配置',
        koishi: '{KoishiServerAddress}',
        koishiEmpty: '服务器地址未配置',
        cmcc: 'API Key 已配置',
        cmccEmpty: 'API Key 未配置',
        webhookEmpty: '尚未添加 Webhook',
      },
    },
    advanced: {
      backupSection: '数据备份',
